...
class ResDataset(Dataset):
    def __init__(self):
        ...
        self.input_words = pad_sequence(self.input_words).T.to(dtype=torch.int32)
        self.output_words = pad_sequence(self.output_words).T.to(dtype=torch.int32)
        self.output_sequence = encoding.vectorize(self.output_words, self.output_max_len,
                                                  self.output_word_to_index)
